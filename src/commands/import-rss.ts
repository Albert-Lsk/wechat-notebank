import { ImportRssArgs } from '../lib/cli';
import { CommandError, getErrorMessage } from '../lib/command-error';
import { parseFeed, FeedKind } from '../lib/feed';
import { BROWSER_USER_AGENT } from '../lib/parser';
...
export interface ImportRssItem {
  title: string;
  link: string;
  pubDate: string | null;
  /** 链接通过安全闸且 host 为 mp.weixin.qq.com 时为 true。 */
  resolvable: boolean;
}

export interface ImportRssResult {
  feedUrl: string;
  feedKind: FeedKind;
  limit: number;
  items: ImportRssItem[];
  /** 仅当列表中存在 resolvable:false 的条目时输出。 */
  note?: string;
}
...
export async function importRssCommand(args: ImportRssArgs): Promise<ImportRssResult> {
  ...
  try {
    ...
    throw new CommandError(
      'CLI_USAGE_ERROR',
      `feed 链接未通过安全校验: ${getErrorMessage(error)}`
    );
  }

  const feedText = await fetchFeedText(feedUrl.toString());

  const outcome = parseFeed(feedText, feedUrl.toString());
  if (!outcome.ok) {
    throw new CommandError('FEED_PARSE_FAILED', outcome.message);
  }

  const items: ImportRssItem[] = outcome.items.slice(0, args.limit).map((item) => ({
    title: item.title,
    link: item.link,
...
async function fetchFeedText(url: string): Promise<string> {
  ...
  try {
    ...
    throw new CommandError('FEED_UNAVAILABLE', `feed 获取失败: ${getErrorMessage(error)}`);
  }
  if (!response.ok) {
    throw new CommandError(
      'FEED_UNAVAILABLE',
      `feed 获取失败（HTTP ${response.status}）: ${url}`
    );
  }
  try {
    return await response.text();
  } catch (error) {
    throw new CommandError('FEED_UNAVAILABLE', `feed 响应读取失败: ${getErrorMessage(error)}`);
  }
}

/** 安全闸 + host 白名单：可归档当且仅当通过校验且为 mp.weixin.qq.com。 */
function isResolvableArticleUrl(rawLink: string): boolean {
  try {
    const parsed = assertSafeArticleUrl(rawLink);
