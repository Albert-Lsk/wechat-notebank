...
- 首版稳定错误码固定为 `CLI_USAGE_ERROR`、`ENV_UNSUPPORTED`、`NODE_VERSION_UNSUPPORTED`、`NPM_NOT_FOUND`、`CHROME_NOT_FOUND`、`CONFIG_INVALID`、`ARTICLE_UNAVAILABLE`、`ARTICLE_PARSE_FAILED`、`MANIFEST_INVALID`、`QUOTE_NOT_FOUND`、`PACK_ALREADY_EXISTS`、`DERIVED_FILE_MODIFIED` 和 `TRANSACTION_FAILED`。
